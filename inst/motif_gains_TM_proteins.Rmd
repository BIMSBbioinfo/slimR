...
volcanoPlot <- function(vars, motifChanges, changeType = 'gained', returnData = FALSE) {

  s <- motifChanges[variantID %in% vars$variantID &  
                  change == changeType & 
                    variant_type %in% c('Polymorphism', 'Disease'),
             length(pos), 
             by = c('variant_type', 'change', 'SLiM')]
  
  ds <- dcast.data.table(s, SLiM ~ variant_type, value.var = 'V1')
  ds[is.na(ds)] <- 0
  
  t <- table(ctm_variants$variant_type)
  ...
  if(returnData == FALSE) {
  ggplot(ds, aes(x = log2(foldChange), y = -log10(pval))) + 
    geom_point(aes(color = pval < 0.05), size = 5, alpha = 0.4) + 
    geom_text_repel(aes(label = ifelse(pval < 0.05, SLiM, '')))
  } else {
    return(ds)
  }
}
...
#all proteins
volcanoPlot(vars = variants, 
            motifChanges = motifChanges)

#all tm proteins
volcanoPlot(vars = variants[variants$uniprotAccession %in% tmProteins,], 
            motifChanges = motifChanges)

#extra cellular regions of transmembrane proteins
volcanoPlot(vars = variants[getVariantsByRegion(variants, etm),], 
            motifChanges = motifChanges)

#cytoplasmic regions of transmembrane proteins
volcanoPlot(vars = variants[getVariantsByRegion(variants, ctm),], 
            motifChanges = motifChanges, changeType = 'lost')
...
l <- list('all_proteins' = volcanoPlot(vars = variants, 
                                  motifChanges = mc, 
                                  changeType = 'gained', returnData = TRUE),
     'tm_proteins' = volcanoPlot(vars =  variants[variants$uniprotAccession %in% tmProteins,], 
                                  motifChanges = mc, 
                                  changeType = 'gained', returnData = TRUE),
     'etm' = volcanoPlot(vars = variants[getVariantsByRegion(variants, etm),], 
                                  motifChanges = mc, 
                                  changeType = 'gained', returnData = TRUE),
     'ctm' = volcanoPlot(vars = variants[getVariantsByRegion(variants, ctm),], 
                                  motifChanges = mc, 
                                  changeType = 'gained', returnData = TRUE))
...
ggplot2::ggplot(data = df,
    ...
    geom_bar(aes(fill = pval < 0.05), position = 'dodge', stat = 'identity') +
